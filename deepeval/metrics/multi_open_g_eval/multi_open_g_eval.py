...
from deepeval.metrics.multi_open_g_eval.template import Multi_Open_G_EvalTemplate
...
LLM_TEST_CASE_PARAMS =["input","actual_output","expected_output","context","retrieval_context"]
...
def extract_test_case_params(test_case: LLMTestCase) -> Dict[str, Optional[object]]:
    result = {}
    
    for key in LLM_TEST_CASE_PARAMS:
        value = getattr(test_case, key, None)
...
class Multi_Open_G_Eval(BaseMetric):
    def __init__(
        self,
        names: List[str],
        evaluation_params: List[LLMTestCaseParams],
        criteria: Optional[List[str]] = None,
        evaluation_steps: Optional[List[List[str]]] = None,
        ...
        if criteria is not None:
            if not all(isinstance(item, str) for item in criteria):
                raise TypeError("All elements in criteria must be strings.")
            if any(item.strip() == '' for item in criteria):
                raise ValueError("Criteria provided cannot contain empty strings.")
    ...
    async def a_measure(
        self,
        test_case: Union[ConversationalTestCase, List[LLMTestCase]],
        ...
        for test in test_case:
            check_llm_test_case_params(test, self.evaluation_params, self)
        ...
        with metric_progress_indicator(
            ...
            for test in test_case:
                ...
                for index,name in enumerate(self.names):
                    ...
                    await self._a_generate_evaluation_steps(index)
        
                    res = await self._a_evaluate(test,index)
                    score_result=res[0]
                    score=float(score_result) / 10
                    score=(0 if self.strict_mode and score < self.threshold else score)
                    if self.verbose_mode:
                        print_intermediate_steps(
                            self.__name__,
                            steps=[
                    ...
                    temp_metric_scores.append({"Metric":name,"Score":score,"Reason":res[1] if len(res) > 1 else "N/A"})     

                test_case_parameters=extract_test_case_params(test)
                average_score = sum(item["Score"] for item in temp_metric_scores) / len(temp_metric_scores) if temp_metric_scores else 0
                final_dict = {**test_case_parameters, "Metrics":temp_metric_scores,"Average":average_score}
                self.score_breakdown.append(final_dict)
                ...
                for metric in self.names:
                    filtered_scores = [
                        metric_score["Score"]
                        for score in self.score_breakdown
                        for metric_score in score["Metrics"]
                        if metric_score["Metric"] == metric
                    ]
                    
                    all_scores.extend(filtered_scores)
                    total_metric_average = sum(filtered_scores) / len(filtered_scores) if filtered_scores else 0
                    self.score_summary.append({"Metric": metric, "Score": total_metric_average})

                total_average = sum(all_scores) / len(all_scores) if all_scores else 0
                self.score_summary.append({"Metric": "Average Score", "Score": total_average})
    ...
    async def _a_generate_evaluation_steps(self,index) -> List[str]:

        is_out_of_index=True             

        if self.evaluation_steps is not None:
            if index < len(self.evaluation_steps):
                is_out_of_index=False
            if not is_out_of_index:
                if self.evaluation_steps[index]!="":
                    return self.evaluation_steps[index]
        else:
        ...
        prompt = Multi_Open_G_EvalTemplate.generate_evaluation_steps(
            criteria=self.criteria[index], parameters=g_eval_params_str, generate_evaluation_steps_prompt_template=self.generate_evaluation_steps_prompt_template
        )
        ...
        if is_out_of_index:
            self.evaluation_steps.append(data["steps"])
        else:
            self.evaluation_steps[index]=(data["steps"])
    ...
    async def _a_evaluate(
        self, test_case: LLMTestCase,index
        ...
        prompt = Multi_Open_G_EvalTemplate.generate_evaluation_results(
            evaluation_steps=self.number_evaluation_steps(index),
            text=text,
    ...
    def number_evaluation_steps(self,index):
        evaluation_steps = """"""
        for index, string in enumerate(self.evaluation_steps[index], start=1):
