...
class MultiMedQAMCQTemplate:
    ...
    @staticmethod
    def generate_output(
        input: str, train_set: object, task:str , n_shots: int
    ):
        ...
        for i in range(n_shots):
            prompt += MultiMedQAMCQTemplate.format_question(train_set[i])
            #print(f"FORMAT QUESTION OF N SHOTS: {i}",MultiMedQAMCQTemplate.format_question(train_set[i]))
        prompt += input
    ...
    def format_question(data: dict, include_answer: bool = True,  ):
        ...
        if include_answer:
            prompt += "{}".format(data["data"]["Correct Option"]) +"<|eot_id|>"
        return prompt
